...
Create `context-os/product/mission.md` with comprehensive product definition following this structure for its' content:
...
Generate `context-os/product/roadmap.md` with an ordered feature checklist:
...
1. **Review the Mission** - Read `context-os/product/mission.md` to understand the product's goals, target users, and success criteria.
...
Create `context-os/product/tech-stack.md` with a list of all tech stack choices that cover all aspects of this product's codebase.
...
Create `context-os/product/tech-stack.md` and populate it with the final list of all technical stack choices, reconciled between the information the user has provided to you and the information found in provided sources.
...
for file in mission.md roadmap.md; do
    if [ ! -f "context-os/product/$file" ]; then
        echo "Error: Missing $file"
    else
        echo "✓ Created context-os/product/$file"
    fi
done

echo "Product planning complete! Review your product documentation in context-os/product/"
```
