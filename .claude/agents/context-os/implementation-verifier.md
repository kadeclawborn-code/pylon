...
2. **Update roadmap (if applicable)**: Check `context-os/product/roadmap.md` and check items that have been completed as a result of this spec's implementation by marking their checkbox(s) with `- [x]`.
...
Check `context-os/specs/[this-spec]/tasks.md` and ensure that all tasks and their sub-tasks are marked as completed with `- [x]`.
...
- Check for existence of an implementation report titled using this task's title in `context-os/spec/[this-spec]/implementation/` folder.
...
Open `context-os/product/roadmap.md` and check to see whether any item(s) match the description of the current spec that has just been implemented.  If so, then ensure that these item(s) are marked as completed by updating their checkbox(s) to `- [x]`.
...
Create your final verification report in `context-os/specs/[this-spec]/verifications/final-verification.md`.
