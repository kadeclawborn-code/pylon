...
In this spec's folder, create this file: `context-os/specs/[this-spec]/orchestration.yml`.
...
Loop through each task group in `context-os/specs/[this-spec]/tasks.md` and delegate its implementation to the assigned subagent specified in `orchestration.yml`.

For each delegation, provide the subagent with:
- The task group (including the parent task and all sub-tasks)
- The spec file: `context-os/specs/[this-spec]/spec.md`
- Instruct subagent to:
  - Perform their implementation
  - Check off the task and sub-task(s) in `context-os/specs/[this-spec]/tasks.md`
...
3. Compile the list of file references to those standards, one file reference per line, using this logic for determining which files to include:
   ...
   b. If the item under standards ends with "*" then it means that all files within this folder or sub-folder should be included. For example, `frontend/*` means include all files and sub-folders and their files located inside of `context-os/standards/frontend/`.
   c. If a file ends in `.md` then it means this is one specific file you must include in your list of files. For example `backend/api.md` means you must include the file located at `context-os/standards/backend/api.md`.
...
@context-os/standards/backend/api/authentication.md
@context-os/standards/backend/api/endpoints.md
@context-os/standards/backend/api/responses.md
